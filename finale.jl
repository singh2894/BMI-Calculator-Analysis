...
# Save one row to CSV beside this script
...
outer = GtkBox(:v)
set_gtk_property!(outer, :halign, GtkAlign.FILL)
set_gtk_property!(outer, :valign, GtkAlign.FILL)
push!(win, outer)
...
set_gtk_property!(title, :halign, GtkAlign.CENTER)
...
set_gtk_property!(subtitle, :halign, GtkAlign.CENTER)
...
set_gtk_property!(grid, :halign, GtkAlign.CENTER)
set_gtk_property!(grid, :valign, GtkAlign.CENTER)
...
mklabel(txt) = begin
    ...
    set_gtk_property!(l, :halign, GtkAlign.END)
    set_gtk_property!(l, :xalign, 1.0)
    l
end
...
# Gender
gender_box = GtkBox(:h); set_gtk_property!(gender_box, :spacing, 20)
male_btn   = GtkRadioButton("Male")
female_btn = GtkRadioButton(male_btn, "Female")
set_gtk_property!(male_btn, :active, true)
push!(gender_box, male_btn); push!(gender_box, female_btn)

# Result
result = GtkLabel("")
set_gtk_property!(result, :use_markup, true)
set_gtk_property!(result, :wrap, true)
set_gtk_property!(result, :justify, GtkJustification.CENTER)
set_gtk_property!(result, :halign, GtkAlign.CENTER)
try set_gtk_property!(result, :xalign, 0.5) catch; end
...
# Align columns
lg = GtkSizeGroup(:horizontal); for w in (age_label, gender_label, height_label, weight_label); push!(lg, w); end
ig = GtkSizeGroup(:horizontal); for w in (age_entry, gender_box, height_entry, weight_entry); push!(ig, w); end
...
# Parse current inputs and save one row (on Enter)
...
for e in (age_entry, height_entry, weight_entry)
    ...
    # Press Enter: refresh + save to CSV
...
signal_connect(win, "key-press-event") do _, ev
    try
        if get(ev, :keyval) == 65307  # Escape
            for e in (age_entry, height_entry, weight_entry); set_gtk_property!(e, :text, ""); end
            set_gtk_property!(male_btn, :active, true)
            set_gtk_property!(result, :label, "")
        end
    catch; end
