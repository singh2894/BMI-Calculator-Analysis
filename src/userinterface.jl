...
function show_ui()
    ...
    # --- Center spacers ---
    top_spacer = GtkLabel("")
    set_gtk_property!(top_spacer, :vexpand, true)
    bottom_spacer = GtkLabel("")
    set_gtk_property!(bottom_spacer, :vexpand, true)
    push!(outer, top_spacer)

    # --- Center container ---
    center_row = GtkBox(:h)
    left_spacer = GtkLabel("")
    set_gtk_property!(left_spacer, :hexpand, true)
    right_spacer = GtkLabel("")
    set_gtk_property!(right_spacer, :hexpand, true)
    ...
    for e in (age_entry, height_entry, weight_entry)
        signal_connect(e, "key-release-event") do _...
            preview()
            false
        end
